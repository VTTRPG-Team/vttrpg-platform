...
export default function GameControls() {
  ...
  const handleStartVote = () => {
    startExitVote()
    setTimeout(() => { console.log("Friend 1 voted"); castVote() }, 2000)
    setTimeout(() => { console.log("Friend 2 voted"); castVote() }, 4000)
  }

  return (
    <>
      {/* Container ปุ่มกด (ใช้ Flex ธรรมดา ไม่ Fix ตำแหน่ง) */}
      ...
      {/* Vote Overlay (Bottom Right Popup) */}
      {voteStatus.isActive && (
        <div className="fixed bottom-20 right-4 w-72 bg-neutral-900 border border-red-500 p-4 rounded-xl shadow-2xl z-[100] pointer-events-auto animate-bounce-in">
          ...
          <div className="w-full bg-gray-700 h-2 rounded-full mb-4 overflow-hidden">
            <div className="bg-red-500 h-full transition-all duration-500" style={{ width: `${(voteStatus.yesVotes / voteStatus.neededVotes) * 100}%` }}></div>
          </div>
          <div className="flex gap-2">
             <button onClick={castVote} className="flex-1 bg-green-600 hover:bg-green-700 text-white py-2 rounded text-xs font-bold">YES</button>
